...
public class UploadController {
    ...
    public ShopHttpResponse upload(@RequestBody MultipartFile[] file) {
        System.out.println(file);
        List list = new ArrayList();
        for (MultipartFile multipartFile:file){
            String url = uploadFile.uploadFileToLocal(multipartFile);
            if ("false".equals(url)){
                return ShopHttpResponse.error("2001","上传失败");
            }
            list.add(url);
        }
        System.out.println("图片上传");
        System.out.println(list);
        return ShopHttpResponse.success(list);
    }
    ...
    public ShopHttpResponse upload(@RequestBody MultipartFile file) {
        ...
        System.out.println("图片上传");
        return ShopHttpResponse.success(url);
    }
